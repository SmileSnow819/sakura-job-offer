...
export function ApplicationForm({
  ...
  const allCompanies = useMemo(
    () => [
      ...data.companies,
      ...catalog.filter(
        (c) => !data.companies.some((local) => local.id === c.id || local.name === c.name),
      ),
    ],
    [data.companies, catalog],
  );
  ...
  const suggestions = allCompanies
    .filter((c) => c.name.toLocaleLowerCase().includes(name.trim().toLocaleLowerCase()))
    .slice(0, 6);
  const submit = () => {
    try {
      ...
      const company: Company = matching
        ? {
            ...matching,
            ...(matching.isCustom ? { name: name.trim(), website: normalizeWebsite(website) } : {}),
          }
        : { id: uid(), name: name.trim(), website: normalizeWebsite(website), isCustom: true };
  ...
  return (
    <Modal
      title={existing ? '编辑投递' : '新增投递'}
      subtitle="公司和岗位必填，其他信息可稍后补充。"
      onClose={onClose}
    >
      <form
      ...
      >
        ...
        {showSuggestions && (!selected || name !== selected.name) && name.trim() && (
          <div className="tracker-suggestions" aria-label="匹配的公司">
            {suggestions.map((c) => (
              <button
                type="button"
                key={c.id}
                onClick={() => {
                  ...
                  setCustom(c.isCustom);
                  setShowSuggestions(false);
                }}
              >
                <CompanyLogo name={c.name} website={c.website} />
                <span>{c.name}</span>
                <small>{c.isCustom ? '我的公司' : '公司库'}</small>
              </button>
            ))}
            {!allCompanies.some((c) => c.name === name.trim()) && (
              <button
                type="button"
                className="tracker-create-company"
                onClick={() => {
                  setCustom(true);
                  setSelected(undefined);
                  setShowSuggestions(false);
        ...
        {(custom ||
          (name.trim() && !selected && !allCompanies.some((c) => c.name === name.trim()))) && (
          <label className="tracker-field">
            官网 <span className="tracker-optional">选填 · 自动获取图标</span>
            <input
              value={website}
              maxLength={2048}
            ...
            />
            <small>搜索不到的公司也可以直接记录，不填官网会使用文字头像。</small>
          </label>
        )}
        {selected && (
          <div className="tracker-company-selected">
            <CompanyLogo
              name={name}
              website={
                selected.isCustom
            ...
            <span>
              {name}
              <small>{selected.isCustom ? '自定义公司' : '来自公司库'}</small>
            </span>
          </div>
        )}
        <label className="tracker-field">
        ...
        {!existing && (
          <p className="tracker-help">
            默认今天投递，从「{data.template[0].name}」开始。保存后可以单独调整这份投递的流程。
          </p>
        )}
        {existing && original?.isCustom && (
          <p className="tracker-help">
            修改这家自定义公司的名称或官网，会同步更新它的其他投递记录。
          </p>
        )}
        {(error || storageError) && (
        ...
        <div className="tracker-actions">
          <button type="button" className="tracker-button" onClick={onClose}>
            取消
          </button>
      ...
      <div className="tracker-detail">
        ...
        {editingFlow ? (
          <FlowEditor
            ...
            onSave={(definitions) => {
              setDraft({ ...draft, stages: applyFlow(draft.stages, definitions) });
          ...
          <div className="tracker-timeline">
            {draft.stages.map((s, i) => (
              <div key={s.id} className={`tracker-timeline-step ${s.status}`}>
                <span className="tracker-timeline-dot">
                  {s.status === 'completed' ? '✓' : s.status === 'skipped' ? '−' : i + 1}
                </span>
                <div className="tracker-stage-content">
                  <div className="tracker-stage-heading">
                    <h4>
                      {s.name}
                      {s.isOffer && <small>OFFER</small>}
                    </h4>
                    <select
                      aria-label={`${s.name}状态`}
                      value={s.status}
                      onChange={(e) =>
                        setDraft({
                          ...draft,
                          stages: changeStage(draft.stages, s.id, e.target.value as StageStatus),
                        })
                      }
                    >
                  ...
                  <div className="tracker-stage-fields">
                    {['completed', 'rejected'].includes(s.status) && (
                      <label>
                        完成 / 结果日期
                        <input
                          type="date"
                          aria-label={`${s.name}完成日期`}
                          value={s.completedAt}
                          onChange={(e) =>
                            setDraft({
                              ...draft,
                              stages: draft.stages.map((stage) =>
                                stage.id === s.id
                                  ? { ...stage, completedAt: e.target.value }
                                  : stage,
                              ),
                            })
                          }
        ...
        <div className="tracker-inline-actions">
          <label className="tracker-check">
            <input
              type="checkbox"
              checked={draft.withdrawn}
              onChange={(e) => setDraft({ ...draft, withdrawn: e.target.checked })}
            />
            已取消投递
          </label>
          <label className="tracker-check">
        ...
        {deleteConfirm && (
          <div className="tracker-warning">
            删除后将移除此岗位的所有阶段、日期和备注。建议先备份。
            <div className="tracker-inline-actions">
              <button className="tracker-button" onClick={() => setDeleteConfirm(false)}>
                保留记录
              </button>
              <button
                className="tracker-button danger"
                onClick={() => {
                  if (!onDelete()) setError('删除失败，原记录已保留。');
                }}
              >
                确认删除这条投递
              </button>
        ...
        <div className="tracker-actions">
          ...
          <div className="tracker-action-spacer" />
          {outcome(draft) === 'active' && currentStage(draft) && (
            <button
              type="button"
