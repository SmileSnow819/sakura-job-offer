...
import { applyFlow, changeStage, currentStage, defaultTemplate, emptyData, makeStages, normalizeWebsite, outcome, parseData, validateFlow } from '../src/features/tracker/model.ts';
import { makeCsv, makeHtml } from '../src/features/tracker/export.ts';

function fixture() {
  const data = emptyData();
  data.companies.push({ id: 'c1', name: '测试公司', website: 'https://example.com', isCustom: true });
  data.applications.push({ id: 'a1', companyId: 'c1', position: '前端开发', appliedAt: '2026-09-03', note: '私人投递备注', stages: makeStages(data.template), archived: false, withdrawn: false, createdAt: '2026-09-03T01:00:00Z', updatedAt: '2026-09-03T01:00:00Z' });
  return data;
}
...
test('推进、跳过、回退和未通过保持单一当前阶段，回退保留备注', () => {
  ...
  assert.deepEqual(stages.slice(0, 3).map(s => s.status), ['skipped', 'skipped', 'active']);
...
test('删除当前阶段自动定位未开始阶段并保留其他历史', () => {
  ...
  app.stages = applyFlow(app.stages, app.stages.filter((_, i) => i !== 1));
...
test('最后一轮完成不等于 offer，只有明确的 offer 阶段完成才计数', () => {
  ...
  app.withdrawn = false; app.stages[0].status = 'skipped';
  assert.equal(outcome(app), 'completed');
});
test('流程校验拒绝空流程、重复名称和非末尾 offer', () => {
  assert.throws(() => validateFlow([]));
  assert.throws(() => validateFlow([{ id: '1', name: '一面', isOffer: false }, { id: '2', name: '一面 ', isOffer: false }]));
  const flow = defaultTemplate(); flow.reverse();
...
test('官网补全协议、移除路径、拒绝危险协议和凭据', () => {
  ...
  for (const website of ['javascript:alert(1)', 'data:text/html,a', 'file:///etc/passwd', 'https://user:pass@example.com', 'invalid']) assert.throws(() => normalizeWebsite(website));
});
...
test('导入拒绝损坏 JSON、不支持的版本、缺失公司和重复 ID', () => {
  ...
  for (const mutate of [d => { d.version = 2; }, d => { d.companies = []; }, d => { d.applications.push(d.applications[0]); }, d => { d.applications[0].stages[1].status = 'active'; }, d => { d.applications[0].appliedAt = '2026-02-30'; }, d => { d.applications[0].stages[0].status = '__proto__'; }]) {
    const data = fixture(); mutate(data); assert.throws(() => parseData(JSON.stringify(data)));
  }
});
test('CSV 支持不同公司流程并默认排除私人备注和官网', () => {
  const data = fixture();
  const second = structuredClone(data.applications[0]); second.id = 'a2'; second.stages[1].name = '四面';
  const records = [data.applications[0], second].map(application => ({ application, company: data.companies[0] }));
